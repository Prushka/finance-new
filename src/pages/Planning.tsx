...
function ScheduleDrawer({
    ...
    return (
        <Drawer
            ...
            position={"bottom"}
            title={s?.auto ? "Imported schedule" :
                viewOnly ? 'Schedule' :
                    s ? 'Edit schedule' : 'Add a schedule'}
        >
            {(s?.auto || viewOnly) ? (
                <div className={"flex flex-col gap-6"}>
                    ...
                    {
                        s?.date && <Row icon={<CalendarIcon/>} title={"Start date"} value={s.date}/>
                    }
                    {
                        s?.endDate && <Row icon={<CalendarIcon/>} title={"End date"} value={s.endDate}/>
                    }
                    {
                        s?.label && <Row icon={<MoreHorizontal/>} title={"Label"} value={s.label}/>
                    }
                    {
                        s?.type && <Row icon={<File/>} title={"Type"} value={s.type}/>
                    }
                    {
                        s?.amount && <Row icon={<DollarSign/>} title={"Amount"} value={`$${s.amount.toFixed(2)}`}/>
                    }
                    {
                        s?.auto && <Row
                            icon={<MoreHorizontal/>}
            ...
            <div className="z-50 sticky top-0 bg-white p-5 border-b-[1px] border-b-gray-300 flex flex-col gap-6">
                <div className="flex justify-between items-center w-full">
                    ...
                    <Button
                        ...
                        variant={"secondary"} className={"flex gap-2"}>
                        <PlusIcon
                        /> Add
                    </Button>
                </div>
                    ...
                    <CardHeader>
                        <div className={"flex justify-between gap-2 items-center"}>
                            <div className={"flex flex-col"}>
                                ...
                                <CardDescription>
                                    See your predicted income & expenses
                                </CardDescription>
                            </div>
                            <TabGroup2
                                selected={t}
                                setSelected={setT}
                                options={["Calendar", "Chart"]}
                            />
                        </div>
                    </CardHeader>
                            ...
                            <Calendar
                                ...
                                renderDay={(date) => {
                                    ...
                                    const ss = schedules.filter(
                                        (a: Schedule) => new Date(a.date).getDate() === day
                                    );
                                    ...
                                    if (new Date().getDate() === day) {
                                        return (
                                            <div
                                                className={
                                                    "bg-gray-200 p-2 w-8 h-8 rounded-xl flex justify-center items-center"
                                                }
                                            >
                                                {day}
                                            </div>
